...
export default function Alert({ isShow = false, title = "", prefixIcon = "", subtitle = "", onClose } : AlertProps) {
    ...
    return (
        <div className={`fixed top-0 left-1/2 z-50 transform -translate-x-1/2 px-3 pt-3 transition-all duration-300 ease-out 
            ...
            <div className="shadow-lg bg-green-50 border flex gap-4 border-l-0 border-slate-800/15 px-5 py-5 w-[300px] sm:w-[450px] rounded-r-md relative">
                ...
                <span className="absolute right-4 top-4 cursor-pointer" onClick={() => onClose(true)}>
                    <X size={19} />
                </span>
            </div>
        </div>
